...
- **FastAPI 0.137.0 broke route enumeration** (PR #44) — `app.routes` in 0.137 includes `_IncludedRouter` objects that lack a `.path` attribute, breaking ~27 smoke tests that enumerate routes via `r.path for r in app.routes`. Root cause: pin `fastapi~=0.132` reads as `>=0.132, <1.0` per PEP 440's two-segment compatible-release operator, so pip pulled in 0.137 the moment it shipped. Tightened to `fastapi~=0.135.0` (locally-tested working version). Dependabot already configured (`.github/dependabot.yml`) — each future FastAPI minor now lands as its own PR with CI verifying it.

### Changed — 2026-06-15 (cosmetic ruff cleanup — issue #29)

- **24 cosmetic ruff errors cleared across 9 files** (PR #45) — F541 empty f-strings (`ruff --fix`); F401 unused imports (dropped `typing.Any` + stale local `import json` in `routes/tasks.py`; converted `# noqa` re-exports in `search/__init__.py` to explicit `__all__`; kept `gssapi.raw as gss_raw` with `# noqa: F401` + comment as an availability probe inside `try/except ImportError`); E402 module-level imports not at top in `gateway/app.py` and `gateway/worker.py` suppressed with `# noqa: E402` + block-level rationale comments (intentional placement next to consumers); F841 unused locals all resolved by deleting dead code (no `# noqa` used) — `current_class`, `passes` x2, `summary`, `increment_session_turn = None` no-op reference, `patterns`, `original_hash`+`body_v1`, `tampered` (kept the call, dropped the binding), `synthetic_id`. `edge_cases` in `crystallization_analyzer.py` was intentionally ignored; replaced with an explanatory comment so the intent is preserved.
...
- **`make briefing` hardcoded repo** — removed `-R jp-cruz/LegionForge` from `gh pr list`; uses `gh` default, survives org transfer.
...
[1.0.0]: https://github.com/jp-cruz/LegionForge/releases/tag/v1.0.0
